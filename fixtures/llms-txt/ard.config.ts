...
//
// A real (non-Vercel) app would typically read this from its own env var instead of a literal —
// this file is evaluated as real code (via jiti), so `siteUrl: process.env.SITE_URL` works with no
// special support from the plugin. It's a literal here only so this fixture's expected output is
// identical everywhere it's built, regardless of what env vars happen to be set.
