...
describe('planMarkdownTwins — user-owned sources and regeneration', () => {
  ...
  it('never sweeps generated non-twin artifacts (auth.md, 404.md) as stale', async () => {
    write('app/docs/page.tsx', 'export default () => null;');
    write('.next/server/app/docs.html', html('Docs'));
    // Generated by auth-md.ts / agent-404.ts on a previous run — carry the marker but are not
    // route twins. No twin plan ever produces them, so listing them stale would delete them every
    // build right before their owners rewrite them (phantom `deleted` churn on build N+1).
