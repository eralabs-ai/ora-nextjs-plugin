...
```sh
npm install --save-dev @ora-ai/ax-nextjs
```
...
- **You have a `public/robots.txt`** → ax _appends_ a `Sitemap:` line (only when a sitemap really
  ...
  `# Added by @ora-ai/ax-nextjs`, and only when they're missing. Existing lines are never modified or
...
not scaffolds** — regenerated every build, marked `generated-by: "@ora-ai/ax-nextjs"` in their
...
### The runtime middleware — `@ora-ai/ax-nextjs/middleware`
